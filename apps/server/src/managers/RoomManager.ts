...
import { AudioSourceSchema, GRID } from "@echowave/shared/types/basic";
import { SendLocationSchema } from "@echowave/shared/types/WSRequest";
...
export class RoomManager {
  ...
  private wsConnections = new Map<string, MyWebSocket>(); // map of clientId -> ws
   ...
   */
  broadcast(message: WSBroadcastType): void {
  ...
  processNTPRequestFrom(clientId: string, clientRTT?: number): void {
    ...
    if (clientRTT !== undefined && clientRTT > 0) {
      const alpha = 0.4; // Smoothing factor - more responsive to RTT changes
      client.rtt =
        client.rtt > 0
   ...
   */
  reorderClients(clientId: string): ClientDataType[] {
    const clients = this.getClients();
    ...
    // Update gains
    this._calculateGainsAndBroadcast();
  ...
  syncClient(ws: MyWebSocket): void {
   ...
   */
  private _calculateGainsAndBroadcast(): void {
    const clients = this.getClients();
