...
export const WebSocketManager = ({
  ...
  const createConnection = () => {
    const SOCKET_URL = `${WS_BASE_URL}?roomId=${roomId}&username=${username}&clientId=${clientId}`;
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        ...
        if (!currentSocket || 
          ...
          console.log('Connection appears open, sending NTP heartbeat...');
          useGlobalStore.getState().sendNTPRequest();
        }
      } else {
        console.log('Tab hidden - connection may be suspended by browser');
      }
    };
