...
const loadAudioSourceUrl = async ({ url }: { url: string }) => {
  ...
  if (arrayBuffer.byteLength === 0) {
    throw new Error(`Audio file is empty: ${url}`);
  }
...
export const useGlobalStore = create<GlobalState>((set, get) => {
  ...
  const loadAudioBuffer = async (url: string) => {
    try {
      console.log(`[Audio] Loading audio from: ${url}`);
      const { audioBuffer } = await loadAudioSourceUrl({ url });
      console.log(
        `[Audio] Successfully loaded audio (${audioBuffer.duration.toFixed(2)}s)`
      );
      ...
      console.error(`[Audio] Failed to load audio source ${url}:`, errorMessage);
      console.error(`[Audio] Error details:`, error);
      // Update the source with error status
      set((currentState) => ({
        audioSources: currentState.audioSources.map((as) =>
          as.source.url === url
            ? { ...as, status: "error" as const, error: errorMessage }
            : as
        ),
      }));
    }
  ...
  const unloadAudioBuffer = (url: string) => {
    console.log(`[Memory] Unloading audio buffer: ${url}`);
    
    // First, get the current source to explicitly clear the buffer reference
    const state = get();
    const sourceToUnload = state.audioSources.find(
      (as) => as.source.url === url && as.status === "loaded"
    );
    
    if (sourceToUnload && sourceToUnload.status === "loaded") {
      // Explicitly remove the buffer reference to allow garbage collection
      const buffer = sourceToUnload.buffer;
      // Clear the buffer (helps GC)
      if (buffer) {
        // No direct way to clear AudioBuffer, but removing all references helps
        console.log(`[Memory] Releasing buffer with ${buffer.duration.toFixed(2)}s duration`);
      }
    }
    
    set((currentState) => ({
      audioSources: currentState.audioSources.map((as) => {
        if (as.source.url === url && as.status === "loaded") {
          // Create a completely new object without the buffer property
          return { 
            source: as.source, 
  ...
  // Manage which songs should be loaded based on current song
  const manageAudioBuffers = async (currentUrl: string) => {
    const state = get();
    const currentIndex = state.audioSources.findIndex(
      (as) => as.source.url === currentUrl
    );

    if (currentIndex === -1) return;

    console.log(`%c[Memory Management]`, 'background: #4CAF50; color: white; padding: 2px 8px; border-radius: 3px;');
    console.log(`📍 Current song index: ${currentIndex} of ${state.audioSources.length}`);

    // Determine which songs should be loaded (current + next 2)
    const shouldBeLoadedIndices = new Set([
      currentIndex,
      currentIndex + 1,
      currentIndex + 2,
    ].filter(i => i >= 0 && i < state.audioSources.length));

    console.log(`✅ Should be loaded: indices [${Array.from(shouldBeLoadedIndices).join(", ")}]`);

    // Unload songs before current (already played)
    let unloadedCount = 0;
    for (let i = 0; i < currentIndex; i++) {
      const source = state.audioSources[i];
      if (source.status === "loaded") {
        unloadAudioBuffer(source.source.url);
        unloadedCount++;
      }
    }

    // Unload songs beyond next 2
    for (let i = currentIndex + 3; i < state.audioSources.length; i++) {
      const source = state.audioSources[i];
      if (source.status === "loaded") {
        unloadAudioBuffer(source.source.url);
        unloadedCount++;
    ...
    if (unloadedCount > 0) {
      console.log(`🗑️  Unloaded ${unloadedCount} song(s) from memory`);
    }

    // Show current memory status
    const loadedCount = state.audioSources.filter(s => s.status === "loaded").length;
    const loadingCount = state.audioSources.filter(s => s.status === "loading").length;
    console.log(`💾 Memory status: ${loadedCount} loaded, ${loadingCount} loading, ${state.audioSources.length - loadedCount - loadingCount} idle/error`);

    // Load current and next 2 songs if not already loaded
    const toLoad: number[] = [];
    shouldBeLoadedIndices.forEach(idx => {
      const source = state.audioSources[idx];
      if (source.status === "idle" || source.status === "error") {
        toLoad.push(idx);
      }
    });

    if (toLoad.length > 0) {
      console.log(`⬇️  Loading ${toLoad.length} song(s) at indices: [${toLoad.join(", ")}]`);
      
      // Load current song first with priority
      if (toLoad.includes(currentIndex)) {
        const source = state.audioSources[currentIndex];
        set((currentState) => ({
          audioSources: currentState.audioSources.map((as, i) =>
            i === currentIndex
              ? { source: as.source, status: "loading" as const }
              : as
          ),
        }));
        await loadAudioBuffer(source.source.url);
      }

      // Load next songs in background
      const nextIndices = toLoad.filter(i => i !== currentIndex);
      nextIndices.forEach(idx => {
        ...
        loadAudioBuffer(source.source.url).catch(err => {
          console.error(`Failed to load song at index ${idx}:`, err);
        });
      });
    }
  };
  ...
  return {
    ...
    playAudio: async (data: {
      ...
      // Get the URL that will be played and manage buffers
      const audioIndex = data.audioIndex ?? state.audioSources.findIndex(
        (as) => as.source.url === state.selectedAudioUrl
      );
      
      if (audioIndex >= 0 && audioIndex < state.audioSources.length) {
        const currentUrl = state.audioSources[audioIndex].source.url;
        // Manage buffers in background - don't block playback
        manageAudioBuffers(currentUrl).catch(err => {
          console.error("Error managing audio buffers:", err);
        });
      }
      ...
      if (data.offset >= audioBuffer.duration) {
        console.error(
          `[Sync] Offset exceeds track duration:`,
          {
            offset: data.offset.toFixed(2),
            duration: audioBuffer.duration.toFixed(2),
            url: audioSourceState.source.url,
            loadedSize: audioBuffer.getChannelData(0).length,
          }
        );
        console.error(
          `Sync offset ${data.offset.toFixed(
            ...
            2
          )}s. Aborting playback. This may indicate the audio file was not fully downloaded.`
        );
        toast.error("Track duration mismatch - file may be incomplete");
        return;
      }
    ...
    skipToNextTrack: (isAutoplay = false) => {
      ...
      // Manage buffers for next song (unload old, load upcoming)
      manageAudioBuffers(nextAudioId).catch(err => {
        console.error("Error managing buffers on skip:", err);
      });
    ...
    async handleSetAudioSources({ sources, currentAudioSource }) {
      ...
      console.log(`%c[Queue Update]`, 'background: #2196F3; color: white; padding: 2px 8px; border-radius: 3px;');
      console.log(`📝 Updating queue with ${sources.length} song(s)`);

      // Determine current index to know which songs should remain loaded
      let currentIndex = -1;
      if (currentAudioSource) {
        currentIndex = sources.findIndex(s => s.url === currentAudioSource);
      } else if (state.selectedAudioUrl) {
        currentIndex = sources.findIndex(s => s.url === state.selectedAudioUrl);
      }

      // Create new audioSources array - keep existing loaded states ONLY for current+next 2
      const newAudioSources: AudioSourceState[] = sources.map((source, index) => {
        // Check if this source already exists
        const existing = state.audioSources.find(
          (as) => as.source.url === source.url
        );
        
        // Should this song be loaded? (current + next 2)
        const shouldBeLoaded = currentIndex >= 0 && 
          index >= currentIndex && 
          index <= currentIndex + 2;

        if (existing) {
          // If it's loaded but shouldn't be, unload it
          if (existing.status === "loaded" && !shouldBeLoaded) {
            console.log(`🗑️  Unloading song at index ${index} (not in active window)`);
            return {
              source,
              status: "idle" as const,
            };
          }
          // Keep existing state (loaded, loading, or error) if it should be loaded
          return existing;
        } else {
          // New source, mark as idle (will be loaded on-demand)
          return {
            source,
            status: "idle" as const,
          };
      ...
      // Update state immediately to show all sources
      set({ audioSources: newAudioSources });
      
      const loadedCount = newAudioSources.filter(s => s.status === "loaded").length;
      console.log(`💤 Queue updated: ${loadedCount} songs loaded, ${sources.length - loadedCount} idle`);
      ...
      // Smart loading: only load current + next 2 songs
      const urlToLoad = currentAudioSource || state.selectedAudioUrl;
      if (urlToLoad) {
        console.log(`[Memory] Smart loading enabled for: ${urlToLoad}`);
        await manageAudioBuffers(urlToLoad);
      } else if (newAudioSources.length > 0) {
        // No current song, load first 3 songs
        console.log(`[Memory] Loading first 3 songs`);
        const firstSong = newAudioSources[0];
        await manageAudioBuffers(firstSong.source.url);
      }
    },
    ...
    getMemoryStats: () => {
      ...
      console.log(`%c[Memory Stats]`, 'background: #FF9800; color: white; padding: 2px 8px; border-radius: 3px;');
      console.log(`Total songs: ${audioSources.length}`);
      
      const stats = {
        loaded: audioSources.filter(s => s.status === "loaded").length,
        loading: audioSources.filter(s => s.status === "loading").length,
        idle: audioSources.filter(s => s.status === "idle").length,
        error: audioSources.filter(s => s.status === "error").length,
      };
      
      console.log(`Status breakdown:`, stats);
      
      // Calculate estimated memory usage
      const loadedSongs = audioSources.filter(s => s.status === "loaded");
      let totalDuration = 0;
      let estimatedMemoryMB = 0;
      
      loadedSongs.forEach(song => {
        if (song.status === "loaded" && song.buffer) {
          const duration = song.buffer.duration;
          const channels = song.buffer.numberOfChannels;
          const sampleRate = song.buffer.sampleRate;
          // Each sample is 4 bytes (Float32)
          const bytes = duration * channels * sampleRate * 4;
          const mb = bytes / (1024 * 1024);
          totalDuration += duration;
          estimatedMemoryMB += mb;
          const displayName = "title" in song.source ? song.source.title : undefined;
          console.log(
            `  - ${displayName || song.source.url || "Unknown"}: ${duration.toFixed(1)}s, ~${mb.toFixed(1)}MB`
          );
        }
      });
      
      console.log(`📊 Total loaded: ${loadedSongs.length} songs, ${totalDuration.toFixed(1)}s, ~${estimatedMemoryMB.toFixed(1)}MB`);
      console.log(`💡 Expected: max 3 songs loaded at any time`);
...
// Export memory stats helper for console debugging
if (typeof window !== 'undefined') {
  const win = window as Window & {
    checkMemory?: () => void;
  };
  win.checkMemory = () => {
    useGlobalStore.getState().getMemoryStats();
  };
  console.log('💡 Tip: Run checkMemory() in console to see memory usage');
}
